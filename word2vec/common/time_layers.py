...
class LSTM:
    def __init__(self, Wx, Wh, b):
        ...
        Parameters
        ----------
        Wx: 輸入`x`用的權重參數（整合4個權重）
        Wh: 隱藏狀態`h`用的權重參數（整合4個權重）
        b: 偏權值（整合4個偏權值）
        '''
        self.params = [Wx, Wh, b]
        self.grads = [np.zeros_like(Wx), np.zeros_like(Wh), np.zeros_like(b)]
...
# ====================================================================== #
# 以下顯示的分層是本書沒有說明的分層執行過程。與處理速度相比，
# 以容易瞭解為優先考量。
#
# TimeSigmoidWithLoss: 時間序列資料用的Sigmoid損失層
# GRU: GRU層
# TimeGRU: 時間序列資料用的GRU層
# BiTimeLSTM: 雙向LSTM層
# Simple_TimeSoftmaxWithLoss：執行簡易TimeSoftmaxWithLoss層
# Simple_TimeAffine: 執行簡易TimeAffine層
# ====================================================================== #
...
class GRU:
    def __init__(self, Wx, Wh):
        ...
        Parameters
        ----------
        Wx: 輸入`x`用的權重參數（整合3個權重）
        Wh: 隱藏狀態`h`用的權重參數（整合3個權重）
        '''
